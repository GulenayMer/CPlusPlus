...
/*   Updated: 2022/11/22 16:07:38 by mgulenay         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::signForm(AForm &form_obj) const
{
	try
	{
		form_obj.beSigned(*this);
		std::cout << YELLOW << this->getName() << " signed the " << form_obj.getName() << RESET << std::endl;
	}
	catch(const std::exception& e)
	{
		std::cout << RED << this->getName() << " could not sign " << form_obj.getName() << " because ..." << e.what() << RESET << std::endl;
	}
	/* if (form_obj.getIfSigned() == true)
		std::cout << YELLOW << this->getName() << " signed the " << form_obj.getName() << RESET << std::endl;
	else
		std::cout << RED <<this->getName() << " could not sign " << form_obj.getName() << " because ..." << RESET << std::endl; */
}

/* this attempts to execute the form */
void Bureaucrat::executeForm(AForm const &form)
{
	...
	{
		std::cerr << RED << this->_name << " could not execute " << form.getName() << " because "
		<< e.what() << RESET << '\n';
	}
